...
#define CONF_KERNEL_IOMMU 1     //---G-G-2022-07-18---
...
#define logw(fmt, arg...) fprintf(stderr, fmt "\n", ##arg)
#define logd(fmt, arg...) fprintf(stderr, fmt "\n", ##arg)
#define logv(fmt, arg...) fprintf(stderr, fmt "\n", ##arg)
...
int sunxi_ion_alloc_open()
{
	...
	g_ion_alloc_context->fd = open(DEV_NAME, O_RDONLY, 0);//O_RDWR O_RDONLY
    ...
    g_ion_alloc_context->cedar_fd = open(CEDAR_DEV_NAME, O_RDONLY, 0);   //----G-G-2022-07-18----
...
#if defined(CONF_KERNEL_IOMMU)
    if (g_ion_alloc_context != NULL   && g_ion_alloc_context->cedar_fd > 0) {
...
#if defined(CONF_KERNEL_IOMMU)
	close(g_ion_alloc_context->cedar_fd);g_ion_alloc_context->cedar_fd = 0;
#endif
...
void* sunxi_ion_alloc_palloc(int size)  //---G-G-2022-07-18---
{
    ...
    if (ret < 0) {
        loge("AW_ION_IOC_NEW_ALLOC error %d \n",ret);
        goto ALLOC_OUT;
    }
    ...
    logd("%d,  phy %x\n",  __LINE__ ,iommu_param.iommu_addr);               //----G-G-2022-07-19----
    ...
    logd("aw_mem_list_add_tail, %d phy %x\n", __LINE__,addr_phy);           //----G-G-2022-07-19----
...
void sunxi_ion_alloc_flush_cache(void* startAddr, int size)     //----G-G-2022-07-18----
{
...
#else
    ret = ioctl(g_ion_alloc_context->cedar_fd, AW_MEM_FLUSH_CACHE_RANGE, &range);   //----G-G-2022-07-18----
#endif
    if (ret)
...
struct SunxiMemOpsS _allocionMemOpsS =
{
    ...
    flush_cache:		sunxi_ion_alloc_flush_cache,                  //---G-G-2022-07-18---
