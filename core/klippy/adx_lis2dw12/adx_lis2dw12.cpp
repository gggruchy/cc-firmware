...
void ADXL345::end_query(std::string name, GCodeCommand &gcmd)
{
    ...
    if (res)
        delete res;
    gcmd.m_respond_info("Writing raw accelerometer data to %s file" + filename.str(), true);  //---??---ADXL345
}
...
void ADXL345::cmd_ACCELEROMETER_MEASURE(GCodeCommand &gcmd)
{
    if (is_measuring())
    {
        ...
        end_query(name, gcmd);
        gcmd.m_respond_info("adxl345 measurements stopped", true); //---??---ADXL345
    }
    else
    {
...
void ADXL345::cmd_ADXL345_DEBUG_READ(GCodeCommand &gcmd)
{
    ...
    gcmd.m_respond_info("ADXL345 REG = 0x%x" + std::to_string(reg) + std::to_string(val), true); //---??---ADXL345
}
