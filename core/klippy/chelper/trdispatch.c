// Trigger sync "trsync" message dispatch  触发同步“trsync”消息分发
//
...
send_trsync_trigger(struct trdispatch_mcu *tdm)   
{
    ...
    uint32_t msg[3] = {           //"trsync_trigger oid=%c reason=%c";
...
static void
send_trsync_set_timeout(struct trdispatch_mcu *tdm)     //"trsync_set_timeout"
{
    uint32_t msg[3] = {                             //"trsync_set_timeout oid=%c clock=%u"
        tdm->set_timeout_msgtag, tdm->trsync_oid, (uint32_t)(tdm->expire_clock)
    };
    struct queue_message *qm = message_alloc_and_encode(msg, ARRAY_SIZE(msg));
                     ...
                     , uint32_t state_msgtag)
{
    ...
    struct queue_message *dummy = message_alloc_and_encode(state_prefix, ARRAY_SIZE(state_prefix));
