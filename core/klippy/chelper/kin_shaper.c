...
// Shift pulses around 'mid-point' t=0 so that the input shaper is an identity transformation for constant-speed motion (i.e. input_shaper(v * T) = v * T)
// 
static void
...
//用输入数据和整形器的卷积计算位置 Calculate the position from the convolution of the shaper with input signal
static inline double
calc_position(struct move *m, int axis, double move_time, struct shaper_pulses *sp)     
{
    double res = 0.;
    ...
    //     double move_time1 = .00006123456;
    //    if(move_time ==  move_time1) //-----G-G-2023-05-10-------------------
    //     {
    //         printf("res:%f a:%f t:%f\n",res,a,t,m->start_pos.x);
    //     }
...
static double
shaper_x_calc_position(struct stepper_kinematics *sk, struct move *m , double move_time)        //-----G-G-2023-05-10-------------------
{
...
{
    struct input_shaper *is = (struct input_shaper *)malloc(sizeof(*is));
