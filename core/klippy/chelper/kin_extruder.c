...
static double
pa_move_integrate(struct move *m, double pressure_advance, double base, double start, double end, double time_offset)
{
    if (start < 0.)
        start = 0.;
    if (end > m->move_t)
...
static double
pa_range_integrate(struct move *m, double move_time, double pressure_advance, double hst)
{
...
void __visible
extruder_set_pressure_advance(struct stepper_kinematics *sk, double pressure_advance, double smooth_time)
{
    struct extruder_stepper *es = container_of(sk, struct extruder_stepper, sk);
