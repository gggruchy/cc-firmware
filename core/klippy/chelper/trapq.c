...
inline double move_get_distance(struct move *m, double move_time)     //s=v0t+0.5*a*t*t
{
...
void trapq_add_move(struct trapq *tq, struct move *m)            //---home-2task-G-G--UI_control_task      //--11-move-2task-G-G--UI_control_task--
{
    // GAM_DEBUG_send_UI("2-148-\n" );
    struct move *tail_sentinel = list_last_entry(&tq->moves, struct move, node);
    ...
    if (prev->print_time + prev->move_t < m->print_time) {
        ...
        list_add_before(&null_move->node, &tail_sentinel->node);       //---14-2task-G-G--UI_control_task--
    }
    // else  if (prev->print_time + prev->move_t > m->print_time) {
    //         GAM_DEBUG_printf("trapq_add_move--prev:%f-----m:%f----------\n",prev->print_time ,m->print_time );
    // }
    list_add_before(&m->node, &tail_sentinel->node);
    tail_sentinel->print_time = 0.;
}

// Free any moves older than `print_time` from the trapezoid velocity queue  从梯形速度队列中释放任何早于 `print_time` 的移动
void __visible
