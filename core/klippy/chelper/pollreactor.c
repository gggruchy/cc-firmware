// Code for dispatching timer and file descriptor events  调度计时器和文件描述符事件的代码
//
...
// Allocate a new 'struct pollreactor' object  分配新调度对象
struct pollreactor *
...
static int pollreactor_check_timers(struct pollreactor *pr, double eventtime, int busy)   //---3-3task-G-G--RW_task-
{
    if (eventtime >= pr->next_timer) {
        ...
        for (i=0; i<pr->num_timers; i++) {
            ...
            if (eventtime >= t) {
                busy = 1;
                t = timer->callback(pr->callback_data, eventtime);   //---4-3task-G-G--RW_task-----retransmit_event--command_event
                timer->waketime = t;
            }
            if (t < pr->next_timer)
                pr->next_timer = t;
...
{
    double eventtime = get_monotonic();        //---2-3task-G-G--RW_task-
    int busy = 1;
    while (! pr->must_exit) {
        ...
        if (ret > 0) {
            ...
            for (; i<pr->num_fds; i++)
                if (pr->fds[i].revents)
                    pr->fd_callbacks[i](pr->callback_data, eventtime);  //---10-3task-G-G--RW_task--input_event----kick_event--
        } else if (ret < 0) {
