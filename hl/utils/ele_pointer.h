/**
 * @file cbd_pointer.h
 * @brief cbd bidirection list module
 * @version 1.0
 * @date 2024-03-26
 * 
 */
 ...
 * 
 */
typedef struct cbd_list_head 
{
    struct cbd_list_head *next, *prev;
...
{\
    type *posnode;\
    while(!cbd_list_empty(&(p)->list_name)) {\
    posnode = cbd_list_entry((&(p)->list_name)->next, type, list_name);\
    cbd_list_del((&(p)->list_name)->next);\
    free(posnode);\
    }\
}
...
{\
    pGetNode = NULL;\
    while(!cbd_list_empty(&(p)->list_name)){\
    pGetNode = cbd_list_entry((&(p)->list_name)->next, type, list_name);\
    break;\
    }\
}
...
{\
    cbd_list_del(&(pDelNode->list_name));\
    free(pDelNode);\
}
 ...
 * 
 */
#define cbd_list_entry(ptr, type, member) \
 ...
 * 
 */
#define cbd_list_for_each(pos, head) \
 ...
 * 
 */
#define cbd_list_for_each_safe(p, n, head) \
 ...
 * @return void 
 */
void cbd_list_del_init(const P_LIST_HEAD pEntry);
