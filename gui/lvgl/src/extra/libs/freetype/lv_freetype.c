...
typedef struct face_refer_tag face_refer_t;
...
typedef struct
{
    ...
    uint16_t style;
    uint16_t height;
    face_refer_t *face_refer;
} lv_font_fmt_ft_dsc_t;

struct face_refer_tag
{
    int ref_index;
    lv_font_fmt_ft_dsc_t *fmt;
};
...
bool lv_freetype_init(uint16_t max_faces, uint16_t max_sizes, uint32_t max_bytes)
{
    ...
    FT_Face face;
    FT_New_Face(library, FONT_RESOURCES_PATH, -1, &face);
    face_number = face->num_faces;
    FT_Done_Face(face);
    printf("total face_number = %d\n", face_number);
    for (int i = 0; i < face_number; i++)
    {
        FT_New_Face(library, FONT_RESOURCES_PATH, i, &face);
        printf("face_index %d family_name %s style_name %s\n", i, face->family_name, face->style_name);
        FT_Done_Face(face);
    }
    return true;
Fail:
                                    ...
                                    FT_Library library_is, FT_Pointer req_data, FT_Face *aface)
{
    ...
    // printf("font_face_requester face_id = %p\n", face_id);
    // lv_font_fmt_ft_dsc_t *dsc = (lv_font_fmt_ft_dsc_t *)face_id;
    face_refer_t *face_ref = (face_refer_t *)face_id;
    lv_font_fmt_ft_dsc_t *dsc = face_ref->fmt;
    ...
    {
        error = FT_New_Face(library, dsc->name, face_ref->ref_index, aface);
    }
    if (error)
    {
                                   ...
                                   lv_font_glyph_dsc_t *dsc_out, uint32_t unicode_letter, uint32_t unicode_letter_next)
{
    ...
    int face_index = 0;
    int find = 0;
    //先根据unicode判断使用哪个face
    face_index = tr_get_face_via_unicode(unicode_letter, dsc->style & FT_FONT_STYLE_BOLD ? 1 : 0);
    if (face_index != -1)
    {
        face_id = (FTC_FaceID)(&dsc->face_refer[face_index]);
        scaler.face_id = face_id;
        if (FTC_Manager_LookupSize(cache_manager, &scaler, &face_size) == 0)
        {
            FT_Face face = face_size->face;
    ...
    if (find == 0)
    {
        for (int i = 0; i < face_number; i++)
        {
            if (i == face_index)
                continue;
            face_index = tr_get_face(i, dsc->style & FT_FONT_STYLE_BOLD ? 1 : 0);
            face_id = (FTC_FaceID)(&dsc->face_refer[face_index]);
            scaler.face_id = face_id;
            // printf("get_glyph_dsc_cb_cache face_id = %p --- i = %d  get_face = %d\n", scaler.face_id, i, tr_get_face(i));
            if (FTC_Manager_LookupSize(cache_manager, &scaler, &face_size) == 0)
            {
                FT_Face face = face_size->face;
                FT_UInt charmap_index = FT_Get_Charmap_Index(face->charmap);
...
static bool lv_ft_font_init_cache(lv_ft_info_t *info)
{
    ...
    dsc->face_refer = lv_mem_alloc(sizeof(face_refer_t) * face_number);
    if (dsc->face_refer == NULL)
    {
        lv_mem_free(dsc);
        return false;
    }
    memset(dsc->face_refer, 0, sizeof(face_refer_t) * face_number);
    for (int i = 0; i < face_number; i++)
    {
        dsc->face_refer[i].fmt = dsc;
        dsc->face_refer[i].ref_index = i;
    }

    /* use to get font info */
    FT_Size face_size;
    struct FTC_ScalerRec_ scaler;
    scaler.face_id = (FTC_FaceID)(&dsc->face_refer[1]);
    scaler.width = info->weight;
    scaler.height = info->weight;
    scaler.pixel = 1;
    // printf("lv_ft_font_init_cache face_id = %p\n", scaler.face_id);
...
void lv_ft_font_destroy_cache(lv_font_t *font)
{
    ...
    if (dsc)
    {
        ...
        FTC_Manager_RemoveFaceID(cache_manager, (FTC_FaceID) & (dsc->face_refer[0]));
        // FTC_Manager_RemoveFaceID(cache_manager, (FTC_FaceID)dsc);
        name_refer_del(dsc->name);
        lv_mem_free(dsc->face_refer);
        lv_mem_free(dsc);
    }
}
