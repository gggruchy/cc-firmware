...
static int save_connected_ssid();
static int clear_saved_ssid();
...
static void app_network_update(widget_t **widget_list)
{
    ...
    if (wifi_password_state == false)
    {
        ...
        if (hl_wlan_get_status() == HL_WLAN_STATUS_CONNECTED && hl_wlan_get_connection(&wlan_cur_connection) != -1
            && !lv_obj_has_flag(widget_list[WINDOW_ID_NETWORK_CONTAINER_LIST]->obj_container[0], LV_OBJ_FLAG_HIDDEN))
        {
            char ipaddr[16] = {0};
...
// brief : 保存连接上的网络，确保重启后将该网络设置为高优先级连接
static int save_connected_ssid()
{
    hl_wlan_connection_t wlan_cur_connection = { 0 };
    if (hl_wlan_get_status() == HL_WLAN_STATUS_CONNECTED && hl_wlan_get_connection(&wlan_cur_connection) != -1)
    {
        if (strncmp(wlan_cur_connection.ssid, get_sysconf()->GetString("system", "ssid_connected", "").c_str(), sizeof(wlan_cur_connection.ssid)) != 0)
        {
            get_sysconf()->SetValue("system", "ssid_connected", wlan_cur_connection.ssid);
            get_sysconf()->WriteIni(SYSCONF_PATH);

            LOG_I("saved ssid : %s\n", get_sysconf()->GetString("system", "ssid_connected", "").c_str());
        }
    }    
    return 0;
}
...
static int clear_saved_ssid()
{
    if (strcmp("", get_sysconf()->GetString("system", "ssid_connected", "").c_str()) != 0)
    {
        get_sysconf()->SetValue("system", "ssid_connected", "");
        get_sysconf()->WriteIni(SYSCONF_PATH);
