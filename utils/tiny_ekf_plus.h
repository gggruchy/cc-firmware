...
/**
 * A header-only class for the Extended Kalman Filter.  Your implementing class
 * should #define the constant N and and then #include <TinyEKF.h>  You will
 * also need to implement a model() method for your application.
 */
class TinyEKF {
 private:
  ekf_t ekf;
 ...
 protected:
   ...
   */
  virtual void model(double fx[Nsta], double F[Nsta][Nsta], double hx[Mobs],
                     double H[Mobs][Nsta]) = 0;

  /**
   * Sets the specified value of the prediction error covariance.
   * <i>P<sub>i,j</sub> = value</i>
   * @param i row index
 ...
 public:
   ...
   */
  double getX(int i) { return this->ekf.x[i]; }
   ...
   * @param value value to set
   */
  void setX(int i, double value) { this->ekf.x[i] = value; }

  /**
    Performs one step of the prediction and update.
   * @param z observation vector, length <i>m</i>
   * @return true on success, false on failure caused by non-positive-definite
   matrix.
   */
  bool step(double *z) {
