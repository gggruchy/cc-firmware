...
#else
char SOCKET_PROGRESS_PATH[256] = {0};
#endif
...
char *get_prog_socket(void)
{
	if (!SOCKET_PROGRESS_PATH || !strlen(SOCKET_PROGRESS_PATH))
	{
		...
		if (!tmpdir)
			tmpdir = "/tmp";
		if (sprintf(SOCKET_PROGRESS_PATH, "%s/%s", tmpdir, SOCKET_PROGRESS_DEFAULT) == -1)
